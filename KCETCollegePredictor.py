...
#Page Configurations
st.set_page_config(page_title="COLLEGEculator",page_icon="",layout="wide",initial_sidebar_state="expanded")
...
#df = pd.read_csv("https://github.com/kiranpatil6663/KCET-College-predictor-2025/blob/main/CET_Database_Final2019.csv", sep='[:,|_()]\s+',names=col_names, header=None, 
        ...
        if(cutoff!=0):
            ...
            if(len(District_List)>0):
                for k in District_List:
                    if(k in location):
                        if(rank<cutoff):      
                            new_row = pd.DataFrame([{'Branch': branch, 'College': college, 'Location': location, 'CET Code': cetcode, 'Cutoff': int(cutoff)}])
            ...
            if(j in lst):
                ...
                if(cutoff1!= 0):
                    ...
                    if(rank<cutoff1):
                        ...
                        new_row = pd.DataFrame([{'Branch': j, 'Cutoff': cutoff1, 'Chances': 'High', 'Difference between your rank and Cutoff': string}])
                        opdfCheckChance = pd.concat([opdfCheckChance, new_row], ignore_index=True)
                    else:
...
def generateOptionEntry():
    ...
    df2=outputdframe.style.set_properties(**{'text-align': 'left'}).set_table_styles([dict(selector='th', props=[('text-align', 'left')])])
    df2.style.set_properties(subset=["Branch", "Location"], **{'text-align': 'left'}).hide(axis="index")
...
st.text("\n")
st.text("\n")
st.text("Note: The predictions made in this app are purely based on KCET 2024 - Second Extended\nRound Cutoff Data.")
st.text("\n")
st.text("Made with ♥ by Kiran Patil  & Team Guided By: Asst. prof. Neela v")
st.text("For any queries, drop in an email to us at kiranpatilkorvi@gmail.com")
st.text("\n")
st.text("\n")
